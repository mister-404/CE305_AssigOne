...
A custom visitor which is a subclass of the base visitor that was generated by the ANTLR4 lexer and parser generation is used to then evaluate and pretty print. There is a separate one for each.



In the visitor, there are overridden methods which link to named components in the grammar file. When it comes to the evaluator, this is where it gets really complex because it becomes recursive. Expressions passed to expressions in different ways all having their own values.
...
A viewer of the tree can be accessed by choosing **d)** when prompted. This is using the ANTLR library and Swing.
